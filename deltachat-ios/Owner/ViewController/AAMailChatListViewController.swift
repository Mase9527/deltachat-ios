//
//  AAMailChatListViewController.swift
//  deltachat-ios
//
//  Created by Gongyonghui on 2026/1/24.
...
class AAMailChatListViewController: UITableViewController {
    ...
    private lazy var proxyShieldButton: UIBarButtonItem = {
        let button = UIBarButtonItem(image: UIImage(systemName: "checkmark.shield"), style: .plain, target: self, action: #selector(AAMailChatListViewController.showProxySettings))
        button.tintColor = DcColors.primary
        return button
    }()
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if let cellData = viewModel?.cellDataFor(section: indexPath.section, row: indexPath.row){
            switch cellData.type {
            case .chat(let chatData):
                if chatData.chatId == DC_CHAT_ID_ARCHIVED_LINK {
                    return ContactCell.cellHeight * 0.7
               ...
               let chat = self.dcContext.getChat(chatId: chatData.chatId)
                let contactIds = chat.getContactIds(self.dcContext)
                if chat.isEncrypted == false {
        ...
        case .chat(let chatData):
            ...
            if chatId == DC_CHAT_ID_ARCHIVED_LINK {
                ...
                let chat = self.dcContext.getChat(chatId: chatId)
                 if chat.isEncrypted == false {
                     chatCell.contentView.isHidden = false
                 }else{
                     chatCell.contentView.isHidden = true
    ...
    public func showArchive(animated: Bool) {
        let controller = AAMailChatListViewController(dcContext: dcContext, dcAccounts: dcAccounts, isArchive: true)
...
extension AAMailChatListViewController: UISearchBarDelegate {
...
extension AAMailChatListViewController: ContactCellDelegate {
...
extension AAMailChatListViewController: ChatListEditingBarDelegate {
...
extension AAMailChatListViewController{
...
extension AAMailChatListViewController:UIPopoverPresentationControllerDelegate{
...
// MARK: - QRCodeDelegate
extension AAMailChatListViewController: QrCodeReaderDelegate {
